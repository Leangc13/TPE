...
AUDIO_RAPIDO = "lapachos_lento_pitch.wav"  # nombre del archivo de voz rápida
...
def punto3(ruta_lenta, ruta_rapida):
    ...
    Figura 1 — Comparación lenta vs pitch (varios períodos):
    ...
    for i, (vl, vr) in enumerate(zip(VOCALES_LENTA, VOCALES_RAPIDA)):
        ...
        fmts_r = _graficar_un_panel(
            ...
            nombre=f"{nombre_vocal} — PITCH  (F₀≈{f0_rapida[i]:.0f} Hz)",
    ...
    plt.savefig("punto3_fft_lenta_vs_pitch.png", dpi=150)
    plt.show()
    print("\nFigura guardada: punto3_fft_lenta_vs_pitch.png")
